...
The functions requires a QGAM model fitted with `qgam::qgam` or `qgam:mqgam` as well as the `pred` argument given for such a call.
...
better_parametric_plot(qgam = tmp.x.1, pred = list(Condition = c("matched", "mismatched")))
```
...
```{r fig.align='center'}
# create an 'old' smooth plot
old.plot <- plot(tmp.x.1, select = 1)
```
